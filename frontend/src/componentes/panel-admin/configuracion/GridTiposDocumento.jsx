...
const GridTiposDocumento = ({ tiposDocumento, cargando, onEditar, onEliminar }) => {
  if (cargando) {
    return (
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 px-4">
        ...
        <p className="text-base text-gray-400 dark:text-gray-500 text-center max-w-sm">
          Agrega tu primer tipo de documento para gestionar tus registros.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {tiposDocumento.map((tipo) => {
        const gradient = getGradientByIndex(tipo.id_tipo_documento);
        const initials = getInitials(tipo.nombre_tipo_documento);
...
export default GridTiposDocumento;