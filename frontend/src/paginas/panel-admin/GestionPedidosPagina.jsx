...
const GestionPedidosPagina = () => {
  ...
  try {
    console.log("Productos del pedido a agregar:", pedidoActivo.productos);
    ...
    for (const p of pedidoActivo.productos) {
      ...
      console.log(`Agregando: ${p.nombre_producto} - Cantidad: ${p.cantidad}`);
      agregarProducto(productoParaStore);
    }
   
    setTimeout(() => {
      const { detalle } = useVentaStore.getState();
      console.log("Detalle después de agregar:", detalle);
      detalle.forEach(item => {
        console.log(`${item.nombre} - Cantidad: ${item.cantidad}`);
      });
    }, 100);
  ...
  const estadoDisplay = (mesa) => mesa.estado_local || mesa.estado_mesa || "disponible";

  const resumen = {
    total: mesas.length,
    disponible: mesas.filter(m => estadoDisplay(m) === "disponible").length,
    ocupada: mesas.filter(m => ["ocupada", "ocupado"].includes(estadoDisplay(m))).length,
    reservada: mesas.filter(m => estadoDisplay(m) === "reservada").length,
    bloqueada: mesas.filter(m => estadoDisplay(m) === "bloqueada").length,
  };

  const totalPedido = pedidoActivo?.productos?.reduce(
    (acc, p) => acc + (p.precio_producto * p.cantidad), 0
  ) ?? 0;

  const tienePedidoActivo = pedidoActivo !== null && pedidoActivo.estado !== "completado";
              ...
              <>
                <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                  <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">
                    🍽️ Pedido Activo #{pedidoActivo.id_pedido}
                  </p>
                  <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                    Fecha: {formatearFecha(pedidoActivo.fecha_pedido)} | Estado: {pedidoActivo.estado}
                  </p>
                </div>

                <div className="mb-4">
                  <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Productos del pedido</h4>
                      ...
                      <div key={i} className="flex items-center justify-between gap-3 py-2 border-b border-gray-100 dark:border-gray-800 last:border-0">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            Cantidad: {p.cantidad} x S/ {p.precio_producto.toFixed(2)}
                          </p>
                        </div>
                        <p className="text-sm font-semibold text-gray-900 dark:text-white">
                          S/ {(p.precio_producto * p.cantidad).toFixed(2)}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white text-sm font-semibold rounded-xl transition-colors"
                  >
                    {completandoPedido ? (
                      <>
